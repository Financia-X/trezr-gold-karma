...
const ComparisonTable = () => {
  ...
  const renderFeatureValue = (value: boolean | string, isCompetitor = false) => {
    if (value === true) {
      ...
      return <span className={`text-sm font-medium ${isCompetitor ? 'text-muted-foreground' : 'text-primary'}`}>
          {value}
        </span>;
    }
  };
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {features.map((row, index) => <tr key={index} className={`border-b border-border hover:bg-muted/20 transition-colors duration-200 ${row.highlight ? 'bg-primary/5' : ''}`}>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className="font-medium">{row.feature}</span>
                          {row.highlight && <Badge variant="secondary" className="text-xs">
                              New
                            </Badge>}
                        </div>
                      </td>
